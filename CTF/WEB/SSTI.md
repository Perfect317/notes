...
<font color=red>**原理**</font>：**模板引擎**是为了使 **用户界面**和 业务数据（内容）分离而产生的，它可以生成特定格式的文档，**利用模板引擎来生成前端的HTML代码，模板引擎会提供一套生成HTML代码的程序，之后只需获取用户的数据，放入渲染函数，该数据便会嵌入生成好的HTML页面中**，然后反馈给浏览器，呈现在用户面前

<font color=red>**防御措施**</font>：最首要做的一项是对用户输入的清理和过滤，这样能大大的降低此类问题带来的安全威胁
另一个选择是创建一个安全加固/沙箱环境，禁用或删除潜在的危险指令

为了防止此类漏洞，最好要像使用eval()函数一样去处理字符串加载功能，尽可能的加载静态模板文件，不让用户输入的代码去执行

注意：我们已经确定此功能类似于require()函数调用。因此，也应该防止本地文件包含（LFI）漏洞。不要允许用户控制此类文件或其内容的路径

另外，无论在何时，如果需要将动态数据传递给模板，不要直接在模板文件中执行，可以使用模板引擎的内置功能来扩展表达式，实现同样的效果
